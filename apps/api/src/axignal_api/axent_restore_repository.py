...
class AxentRestoreRepository(AxentActionRepository):
    def restore_workspace(
        ...
        *,
        ...
        with self._cursor(role="axignal_app", tenant_id=tenant_id) as cursor:
            cursor.execute(
                """
                SELECT * FROM tenant_private.support_tool_invocations
                WHERE tenant_id = %s
                  AND tool_name = 'restore_workspace'
            ...
            if workspace["state"] != "CLOSED":
                raise ValueError("workspace_not_closed")

            cursor.execute(
                """
                SELECT action_id
                FROM tenant_private.support_actions
                WHERE tenant_id = %s
            ...
            archive_action = cursor.fetchone()
            if archive_action is None:
                raise ValueError("workspace_archive_receipt_missing")
            ...
            cursor.execute(
                ...
                INSERT INTO tenant_private.support_tool_invocations (
                  ...
                  %s, %s, %s, 'restore_workspace', 'v1', %s, %s, %s,
            ...
            cursor.execute(
                ...
                WHERE tenant_id = %s
                  ...
                  AND action_type = 'restore_workspace'
            ...
            cursor.execute(
                ...
                WHERE tenant_id = %s AND workspace_id = %s AND state = 'CLOSED'
            ...
            if after is None:
                raise ValueError("workspace_restore_conflict")
            ...
            cursor.execute(
                ...
                INSERT INTO tenant_private.support_actions (
                  ...
                  %s, %s, %s, 'restore_workspace', 'WORKSPACE', %s, %s, %s,
                  'STEP_UP_AUTH', %s, 'NOT_APPLICABLE'
                )
                RETURNING *
                """,
                (
            ...
            result = {
                ...
                "rollback_of": str(archive_action["action_id"]),
            }
            cursor.execute(
                """
