...
first_available_date = legacy.first_available_date
html_text = legacy.html_text
parse_release_calendar = legacy.parse_release_calendar
percentile = legacy.percentile


def bounded_date_query(lower: date, upper: date, *, sort: bool = False) -> str:
    if lower > upper:
    ...
    if sort:
        query += " SORT BY publication-number ASC"
    return query
...
def _search_interval(
    ...
    *,
    ...
    return legacy._run_search(
        plan,
        bounded_date_query(lower, upper, sort=sort),
        budget=budget,
    )
...
def _history_report(
    ...
    *,
    ...
    return {
        "schema_version": "axignal.o01-history-depth-report/v0.2",
        "status": "PASS",
        ...
        "search_scope": "ALL",
        "query_contract": "CANONICAL_CLOSED_PUBLICATION_DATE_INTERVALS",
...
def _frequency_and_lag(
    ...
    *,
    ...
    frequency = {
        "schema_version": "axignal.o01-update-frequency-report/v0.2",
        "status": "PASS",
    ...
    lag = {
        "schema_version": "axignal.o01-publication-lag-report/v0.2",
        "status": "PASS",
        "metric_semantics": (
...
def run_campaign(plan_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    legacy.write_json(
        ...
        {
            "schema_version": "axignal.o01-official-source-observations/v0.2",
            "status": "PASS",
            "documents": official,
            "fabricated_evidence": 0,
        },
    ...
    for year in plan["release_calendar_years"]:
        ...
        content_type = (metadata.get("content_type") or "").casefold()
        if "text/html" in content_type or body.lstrip().startswith(b"<!DOCTYPE html"):
            raise O01QualityCampaignError(
                f"Release calendar {year} returned HTML instead of CSV"
            )
        ...
        calendar_observations.append(
            {
                ...
                "body_sha256": legacy.sha256_prefixed(body),
    ...
    legacy.write_json(
        ...
        {
            "schema_version": "axignal.o01-release-calendar-observations/v0.2",
            "status": "PASS",
    ...
    result = {
        "schema_version": "axignal.o01-history-frequency-lag-result/v0.2",
