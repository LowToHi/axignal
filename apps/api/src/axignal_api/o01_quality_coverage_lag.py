...
def coverage_report(
    *,
    ...
    cpv_prefixes = Counter(
        code[:2] for record in normalized_records for code in record.cpv_codes if len(code) >= 2
    )
    procedure_types = Counter(
        value for record in normalized_records for value in record.procedure_types
    )
    notice_types = Counter(value for record in normalized_records for value in record.notice_type)
    form_types = Counter(value for record in normalized_records for value in record.form_type)
    ...
    publication_dates = sorted(
        record.publication_date for record in normalized_records if record.publication_date
    )
    ...
    return {
        ...
        "countries_and_jurisdictions_observed": {
            ...
            "place_of_performance_country": dict(sorted(performance_countries.items())),
        ...
        "lots": {
            "records_with_lot_identifiers": sum(bool(item.lot_identifiers) for item in normalized_records),
            "lot_identifier_count": sum(len(item.lot_identifiers) for item in normalized_records),
        },
        "history_available": {
            "sample_earliest_publication_date": publication_dates[0] if publication_dates else None,
            "sample_latest_publication_date": publication_dates[-1] if publication_dates else None,
        ...
        "frequency": {
            ...
            "maximum_observed_calendar_gap_days": max(observed_gaps) if observed_gaps else None,
        },
        ...
        "areas_not_covered": [
            ...
            "Notices outside 2026-07-01 through 2026-07-31 and notices outside TED ACTIVE scope.",
            "National and sub-threshold procurement portals not published through TED.",
            "Attachments, source-native full text, third-party works and full XML/HTML payloads.",
            "Natural-person buyers, personal contact endpoints and contact values in retained evidence.",
            "Exhaustive archive history; the history probe is a bounded API observation only.",
            "Source factual correctness beyond transformation fidelity of projected TED fields.",
        ],
    }
...
def lag_report(
    ...
    *,
    ...
    for record in normalized_records:
        ...
        normalised_at = datetime.fromisoformat(record.normalised_at.replace("Z", "+00:00"))
        ...
        acquisition.append(
            acquisition_by_notice.get(
                ...
                max(0.0, (retrieved_completed - retrieved_started).total_seconds()),
            )
        )
        normalisation.append(max(0.0, (normalised_at - retrieved_completed).total_seconds()))
    ...
    return {
        ...
        "metrics": {
            "source_publication_lag": metric_summary(source_publication, unit="seconds"),
            "source_availability_lag": metric_summary(
                source_availability_upper_bound, unit="seconds_upper_bound"
            ),
            "AXIGNAL_acquisition_lag": metric_summary(acquisition, unit="seconds"),
            "normalisation_lag": metric_summary(normalisation, unit="seconds"),
            "indexing_lag": metric_summary(indexing, unit="seconds"),
            "subscriber_notification_lag": metric_summary(notification, unit="seconds"),
        },
        "confidence_limitations": [
            "TED publication-date is date-granular, so source_publication_lag uses 00:00 UTC as a conservative reference.",
            "Historical one-shot acquisition cannot observe the exact first API-availability instant; source_availability_lag is an upper bound ending at first campaign observation.",
            "AXIGNAL acquisition measures the bounded Search API HTTP request, not recurring production polling cadence.",
            "Subscriber notification lag measures creation of the private internal notification ledger, not external email, webhook or message delivery.",
            "Percentiles describe this frozen stratified sample and are not population-wide confidence intervals.",
        ],
    }
