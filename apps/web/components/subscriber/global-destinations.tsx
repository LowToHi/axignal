...
function GenericDestination({ kind, onThemeChange }: { kind: string; onThemeChange: (theme: "dark" | "light") => void }) {
  ...
  if (kind === "settings") return <div className={styles.page}><PageHeader eyebrow="Organisation controls" title="Settings" description="Locale, appearance, notifications, privacy and security are explicit and reversible."/><section className={styles.section}><div className={styles.sectionHeader}><h2>APPEARANCE</h2><span>First-class theme parity</span></div><div className={styles.dataRow}><span>THEME</span><strong>Workspace appearance</strong><small>Stored on this device; server preference reconciliation is pending</small><div className={styles.inlineActions}><button type="button" onClick={() => onThemeChange("dark")}>Dark</button><button type="button" onClick={() => onThemeChange("light")}>Light</button></div></div></section><section className={styles.section}><div className={styles.sectionHeader}><h2>PRIVACY & SECURITY</h2><span>Server authoritative</span></div><div className={styles.sideSummary}><article><span>PRIVATE KNOWLEDGE</span><strong>Explicit use only</strong><p>Private tenant context is never included in a ResearchRun without an explicit scoped choice.</p></article><article><span>SESSION</span><strong>Passkey-ready · bounded session</strong><p>Step-up is required for consequential organisation and external-handoff operations.</p></article></div></section></div>;
