...
CREATE TABLE IF NOT EXISTS tenant_private.subscriber_workspaces (
  ...
  research_run_id uuid NOT NULL REFERENCES tenant_private.research_runs(research_run_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS tenant_private.subscriber_workspace_documents (
  ...
  tenant_id uuid NOT NULL,
  workspace_id uuid NOT NULL REFERENCES tenant_private.subscriber_workspaces(workspace_id) ON DELETE RESTRICT,
  title text NOT NULL CHECK (char_length(title) BETWEEN 1 AND 300),
...
CREATE TABLE IF NOT EXISTS tenant_private.subscriber_workspace_exports (
  ...
  tenant_id uuid NOT NULL,
  workspace_id uuid NOT NULL REFERENCES tenant_private.subscriber_workspaces(workspace_id) ON DELETE RESTRICT,
  document_id uuid REFERENCES tenant_private.subscriber_workspace_documents(document_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS tenant_private.subscriber_workspace_audit_events (
  ...
  tenant_id uuid NOT NULL,
  workspace_id uuid REFERENCES tenant_private.subscriber_workspaces(workspace_id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS subscriber_workspace_documents_workspace_idx
  ON tenant_private.subscriber_workspace_documents (tenant_id, workspace_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS subscriber_workspace_exports_workspace_idx
  ON tenant_private.subscriber_workspace_exports (tenant_id, workspace_id, created_at DESC);
CREATE INDEX IF NOT EXISTS subscriber_workspace_audit_idx
  ON tenant_private.subscriber_workspace_audit_events (tenant_id, occurred_at DESC);
...
DROP POLICY IF EXISTS subscriber_workspaces_tenant_policy ON tenant_private.subscriber_workspaces;
...
DROP POLICY IF EXISTS subscriber_workspace_documents_tenant_policy ON tenant_private.subscriber_workspace_documents;
CREATE POLICY subscriber_workspace_documents_tenant_policy
  ON tenant_private.subscriber_workspace_documents
  USING (tenant_id = tenant_private.current_tenant_id())
  WITH CHECK (tenant_id = tenant_private.current_tenant_id());

DROP POLICY IF EXISTS subscriber_workspace_exports_tenant_policy ON tenant_private.subscriber_workspace_exports;
CREATE POLICY subscriber_workspace_exports_tenant_policy
  ON tenant_private.subscriber_workspace_exports
  USING (tenant_id = tenant_private.current_tenant_id())
  WITH CHECK (tenant_id = tenant_private.current_tenant_id());

DROP POLICY IF EXISTS subscriber_workspace_audit_tenant_policy ON tenant_private.subscriber_workspace_audit_events;
...
GRANT SELECT, INSERT, UPDATE ON tenant_private.subscriber_workspaces TO axignal_app;
GRANT SELECT, INSERT, UPDATE ON tenant_private.subscriber_workspace_documents TO axignal_app;
GRANT SELECT, INSERT ON tenant_private.subscriber_workspace_exports TO axignal_app;
GRANT SELECT, INSERT ON tenant_private.subscriber_workspace_audit_events TO axignal_app;
REVOKE DELETE ON tenant_private.subscriber_workspaces FROM axignal_app;
REVOKE DELETE ON tenant_private.subscriber_workspace_documents FROM axignal_app;
REVOKE UPDATE, DELETE ON tenant_private.subscriber_workspace_exports FROM axignal_app;
REVOKE UPDATE, DELETE ON tenant_private.subscriber_workspace_audit_events FROM axignal_app;
