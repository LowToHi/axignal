...
class AuthorityExtractionError(RuntimeError):
    """Raised when typed F01 human authority cannot be evaluated safely."""
    ...
    for comment in comments:
        ...
        for candidate in candidate_objects(str(comment.get("body") or "")):
            ...
            try:
                ...
                continue
            if decision.technical_head_sha != expected_technical_head_sha:
                continue
    ...
    *,
    ...
    for authority in sorted(REQUIRED_AUTHORITIES):
        decision = extract_latest_verified(
            ...
            authority=authority,
            expected_technical_head_sha=expected_head,
    ...
    result = result_payload(
        ...
        technical_head_sha=expected_head,
        evidence_expires_at=parse_utc(evidence["evidence_expires_at"]),
    ...
    envelope_path = output_dir / "rights-authority-envelope.v0.1.json"
    if evaluation.campaign_authorised:
        envelope = {
            "schema_version": "axignal.f01-rights-authority-envelope/v0.1",
            "manifest_reference": manifest_reference,
            "technical_head_sha": expected_head,
            ...
            "output": "F01_PRIVATE_CAMPAIGN_AUTHORISED",
        }
...
def main() -> int:
    ...
    if args.require_authorised and not result["campaign_authorised"]:
        return 2
    return 0
