...
from verify_gate7_o01_v0_4_scope_contract import materialize as materialize_contract

ROOT = Path(__file__).resolve().parents[1]
MANIFEST = (
    ROOT
    / "data/acceptance/approvals/"
    "AX-LIB-O01-campaign-authority-manifest.v0.4.json"
)
MANIFEST_SHA = "sha256:fb4de11ed01dbadb76b8e8d91474a482d3011fa8128b99adcf5c1fe0363c3983"


class PlanMaterializationError(RuntimeError):
    """Raised when the v0.4 plan cannot be derived exactly."""
...
def materialize_plan() -> tuple[dict[str, Any], dict[str, Any]]:
    require(sha256_path(MANIFEST) == MANIFEST_SHA, "v0.4 authority-manifest digest drift")
    manifest = load(MANIFEST)
    materialized_contract, _delta = materialize_contract()

    plan = deepcopy(materialized_contract)
    plan["schema_version"] = "axignal.o01-quality-coverage-lag-plan/v0.4"
    ...
    require(
        ...
        "v0.4 plan differs from exact materialized contract",
    )
    require(plan["source"]["scope"] == "ALL", "Historical ALL scope is not materialized")
    require(
...
def main() -> int:
    ...
    result = {
        "status": "PASS",
        "output": "O01_V0_4_CAMPAIGN_PLAN_MATERIALIZED",
