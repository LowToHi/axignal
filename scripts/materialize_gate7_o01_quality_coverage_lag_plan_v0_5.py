...
from verify_gate7_o01_v0_5_query_contract import materialize as materialize_contract

ROOT = Path(__file__).resolve().parents[1]
MANIFEST = (
    ROOT
    / "data/acceptance/approvals/"
    "AX-LIB-O01-campaign-authority-manifest.v0.5.json"
)
MANIFEST_SHA = "sha256:57fb560f5f04cc994180bc8ddfd11f31dc88d97a45ad71e76496906bad02c1c1"


class PlanMaterializationError(RuntimeError):
    """Raised when the v0.5 plan cannot be derived exactly."""
...
def materialize_plan() -> tuple[dict[str, Any], dict[str, Any]]:
    require(sha256_path(MANIFEST) == MANIFEST_SHA, "v0.5 authority-manifest digest drift")
    manifest = load(MANIFEST)
    materialized_contract, _delta = materialize_contract()

    plan = deepcopy(materialized_contract)
    plan["schema_version"] = "axignal.o01-quality-coverage-lag-plan/v0.5"
    ...
    require(
        ...
        "v0.5 plan differs from exact materialized contract",
    )
    ...
    require(
        ...
        "Canonical date interval is not materialized",
    )
    require(
...
def main() -> int:
    ...
    result = {
        "status": "PASS",
        "output": "O01_V0_5_CAMPAIGN_PLAN_MATERIALIZED",
