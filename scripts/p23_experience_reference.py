...
def claim_decision(
    *,
    ...
    conditional_uses = {
        "CONDITIONAL",
        "ALLOWED_WITH_SCOPE",
        "ALLOWED_AS_DESIGN_OBJECTIVE",
    }
    if public_use in conditional_uses and not limitations_present:
...
def publication_decision(
    *,
    gates: dict[str, bool],
    human_publication_authority: bool,
) -> str:
    if not gates or not all(gates.values()):
        return "BLOCK"
    if human_publication_authority:
        return "ALLOW_PUBLICATION"
    return "HUMAN_APPROVAL_REQUIRED"


def analytics_decision(
    *,
    event_name: str,
...
def experiment_decision(
    *,
    ...
    sample_declared: bool,
    stopping_rule_declared: bool,
    human_growth_approval: bool,
) -> str:
    ...
    if not sample_declared or not stopping_rule_declared:
        return "DENY_METHOD"
    if human_growth_approval:
        return "ALLOW_BOUNDED_EXPERIMENT"
    return "HUMAN_APPROVAL_REQUIRED"


def marketing_scale_decision(
    *,
    conversion_validated: bool,
    contribution_positive: bool,
    budget_within_limit: bool,
    human_budget_approval: bool,
) -> str:
    if not conversion_validated:
        return "HOLD"
    if not contribution_positive or not budget_within_limit:
        return "HOLD"
    if human_budget_approval:
        return "ALLOW_BOUNDED_SCALE"
    return "HUMAN_APPROVAL_REQUIRED"


def accessibility_release(
    *,
...
def performance_release(
    *,
    ...
    passed = (
        lcp_ms <= budgets["landing_lcp_ms"]
        and cls <= budgets["landing_cls"]
        and inp_ms <= budgets["landing_inp_ms"]
    )
    return "PASS" if passed else "BLOCK"


def pricing_presentation(
    *,
    server_amount_minor: int,
    displayed_amount_minor: int,
    server_currency: str,
    displayed_currency: str,
) -> str:
    if server_amount_minor != displayed_amount_minor:
        return "BLOCK_PRICE_MISMATCH"
    if server_currency != displayed_currency:
        return "BLOCK_PRICE_MISMATCH"
    return "PASS"
