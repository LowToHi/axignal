...
TARGET_HEAD="a525cef3019f004af0c278a92b8afc2403e7ec9a"
MANIFEST_REFERENCE="sha256:fb4de11ed01dbadb76b8e8d91474a482d3011fa8128b99adcf5c1fe0363c3983"
EXPECTED_BRANCH="agent/ax-gate7-o01-v04-all-scope-campaign"
CAMPAIGN_ID="AX-LIB-O01-TED-QALAG-ML-CONTROLS-v0.4"
...
jq -e \
  ...
  '.schema_version == "axignal.o01-real-campaign-execution-request/v0.4" and
...
python -m compileall -q \
  ...
  scripts/materialize_gate7_o01_quality_coverage_lag_plan_v0_4.py \
  scripts/measure_gate7_o01_multilingual_journeys.py \
  scripts/rehearse_gate7_o01_controls.py \
  scripts/run_gate7_o01_quality_coverage_lag_campaign_v0_4.py \
  scripts/verify_gate7_o01_quality_coverage_lag_contract_v0_4.py \
  scripts/verify_gate7_o01_v0_4_scope_contract.py
ruff check \
  apps/api/src/axignal_api/gate7_o01_controls.py \
  apps/api/src/axignal_api/gate7_o01_multilingual.py \
  apps/api/src/axignal_api/gate7_o01_runtime.py \
...
python scripts/verify_gate7_o01_v0_4_scope_contract.py \
  --materialized-output "$RESULT_DIR/materialized-execution-contract.v0.4.json" \
  | tee "$RESULT_DIR/scope-contract-result.json"
python scripts/materialize_gate7_o01_quality_coverage_lag_plan_v0_4.py \
  --output "$PLAN_PATH" | tee "$RESULT_DIR/plan-materialization-result.json"
python scripts/verify_gate7_o01_quality_coverage_lag_contract_v0_4.py \
  --plan "$PLAN_PATH" | tee "$RESULT_DIR/plan-contract-result.json"
jq -e '.source.scope == "ALL" and .source.source_state == "CANDIDATE"' "$PLAN_PATH"
...
scope_id="$(jq -r '.scope_evidence.artifact_id' "$AUTHORITY_MANIFEST")"
scope_digest="$(jq -r '.scope_evidence.artifact_digest' "$AUTHORITY_MANIFEST")"
gh api "/repos/${GITHUB_REPOSITORY}/actions/artifacts/${scope_id}" \
  > "$RESULT_DIR/active-scope-artifact-metadata.json"
jq -e --arg digest "$scope_digest" \
  '.expired == false and .digest == $digest' \
  "$RESULT_DIR/active-scope-artifact-metadata.json"

gh api --paginate \
  "/repos/${GITHUB_REPOSITORY}/issues/124/comments?per_page=100" \
  --slurp | jq '[.[][]]' > /tmp/o01-v0-4-legal.json
gh api --paginate \
  "/repos/${GITHUB_REPOSITORY}/issues/125/comments?per_page=100" \
  --slurp | jq '[.[][]]' > /tmp/o01-v0-4-privacy.json
python scripts/extract_gate7_o01_campaign_authority.py \
  --manifest "$AUTHORITY_MANIFEST" \
  --legal-comments /tmp/o01-v0-4-legal.json \
  --privacy-comments /tmp/o01-v0-4-privacy.json \
...
receipt = {
    "schema_version": "axignal.o01-network-authority-gate/v0.4",
    "status": "PASS",
...
(result_dir / "network-authority-gate.v0.4.json").write_text(
...
python scripts/rehearse_gate7_o01_controls.py preflight \
  ...
  --checkpoint "$RESULT_DIR/pre-campaign-boundary.v0.4.json" \
  --output "$RESULT_DIR/operational-controls-preflight.v0.4.json" \
...
jq -e \
  ...
  "$RESULT_DIR/operational-controls-preflight.v0.4.json"

test ! -e "$RAW_TMP_DIR"
python scripts/run_gate7_o01_quality_coverage_lag_campaign_v0_4.py \
  --plan "$PLAN_PATH" \
...
tar --sort=name --mtime='@0' --owner=0 --group=0 --numeric-owner \
  -cf - -C "$RAW_TMP_DIR" . | gzip -n > /tmp/o01-v0-4-raw.tar.gz
plaintext_sha="sha256:$(sha256sum /tmp/o01-v0-4-raw.tar.gz | awk '{print $1}')"
openssl cms -encrypt -binary -aes256 \
  -in /tmp/o01-v0-4-raw.tar.gz -outform DER \
...
PY
rm -rf "$RAW_TMP_DIR" /tmp/o01-v0-4-raw.tar.gz
test ! -e "$RAW_TMP_DIR"

python scripts/rehearse_gate7_o01_controls.py finalise \
  --preflight "$RESULT_DIR/operational-controls-preflight.v0.4.json" \
  --checkpoint "$RESULT_DIR/pre-campaign-boundary.v0.4.json" \
