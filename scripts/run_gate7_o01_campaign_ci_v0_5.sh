...
TARGET_HEAD="2f3f653c611273db0d461bb2fdad4ee7b3837028"
MANIFEST_REFERENCE="sha256:57fb560f5f04cc994180bc8ddfd11f31dc88d97a45ad71e76496906bad02c1c1"
EXPECTED_BRANCH="agent/ax-gate7-o01-v05-canonical-date-range"
CAMPAIGN_ID="AX-LIB-O01-TED-QALAG-ML-CONTROLS-v0.5"
QUERY="buyer-country IN ({country}) AND publication-date = (20260701 <> 20260731) SORT BY publication-number"
...
jq -e \
  ...
  --arg query "$QUERY" \
  '.schema_version == "axignal.o01-real-campaign-execution-request/v0.5" and
...
python scripts/verify_gate7_o01_v0_5_query_contract.py \
  --materialized-output "$RESULT_DIR/materialized-execution-contract.v0.5.json" \
  | tee "$RESULT_DIR/query-contract-result.json"
python scripts/materialize_gate7_o01_quality_coverage_lag_plan_v0_5.py \
  --output "$PLAN_PATH" | tee "$RESULT_DIR/plan-materialization-result.json"
python scripts/verify_gate7_o01_quality_coverage_lag_contract_v0_5.py \
  --plan "$PLAN_PATH" | tee "$RESULT_DIR/plan-contract-result.json"
jq -e --arg query "$QUERY" \
  '.source.scope == "ALL" and .source.source_state == "CANDIDATE" and .sampling.query_contract == $query' \
  "$PLAN_PATH"
...
query_id="$(jq -r '.query_evidence.artifact_id' "$AUTHORITY_MANIFEST")"
query_digest="$(jq -r '.query_evidence.artifact_digest' "$AUTHORITY_MANIFEST")"
gh api "/repos/${GITHUB_REPOSITORY}/actions/artifacts/${query_id}" \
  > "$RESULT_DIR/query-evidence-artifact-metadata.json"
jq -e --arg digest "$query_digest" \
  '.expired == false and .digest == $digest' \
  "$RESULT_DIR/query-evidence-artifact-metadata.json"

gh api --paginate \
  "/repos/${GITHUB_REPOSITORY}/issues/124/comments?per_page=100" \
  --slurp | jq '[.[][]]' > /tmp/o01-v0-5-legal.json
gh api --paginate \
  "/repos/${GITHUB_REPOSITORY}/issues/125/comments?per_page=100" \
  --slurp | jq '[.[][]]' > /tmp/o01-v0-5-privacy.json
python scripts/extract_gate7_o01_campaign_authority.py \
  --manifest "$AUTHORITY_MANIFEST" \
  --legal-comments /tmp/o01-v0-5-legal.json \
  --privacy-comments /tmp/o01-v0-5-privacy.json \
...
receipt = {
    "schema_version": "axignal.o01-network-authority-gate/v0.5",
    "status": "PASS",
    ...
    "source_scope": "ALL",
    "canonical_date_range": True,
    "source_state": "CANDIDATE",
    "public_launch": "NO_GO",
}
(result_dir / "network-authority-gate.v0.5.json").write_text(
...
python scripts/rehearse_gate7_o01_controls.py preflight \
  ...
  --checkpoint "$RESULT_DIR/pre-campaign-boundary.v0.5.json" \
  --output "$RESULT_DIR/operational-controls-preflight.v0.5.json" \
...
jq -e \
  ...
  "$RESULT_DIR/operational-controls-preflight.v0.5.json"

test ! -e "$RAW_TMP_DIR"
python scripts/run_gate7_o01_quality_coverage_lag_campaign_v0_5.py \
  --plan "$PLAN_PATH" \
...
tar --sort=name --mtime='@0' --owner=0 --group=0 --numeric-owner \
  -cf - -C "$RAW_TMP_DIR" . | gzip -n > /tmp/o01-v0-5-raw.tar.gz
plaintext_sha="sha256:$(sha256sum /tmp/o01-v0-5-raw.tar.gz | awk '{print $1}')"
openssl cms -encrypt -binary -aes256 \
  -in /tmp/o01-v0-5-raw.tar.gz -outform DER \
...
PY
rm -rf "$RAW_TMP_DIR" /tmp/o01-v0-5-raw.tar.gz
test ! -e "$RAW_TMP_DIR"

python scripts/rehearse_gate7_o01_controls.py finalise \
  --preflight "$RESULT_DIR/operational-controls-preflight.v0.5.json" \
  --checkpoint "$RESULT_DIR/pre-campaign-boundary.v0.5.json" \
