...
from axignal_api.o01_history_frequency_lag_v7 import run_campaign
from axignal_api.o01_official_source_retry import OfficialSourceFetchError
from axignal_api.o01_quality_common import O01QualityCampaignError


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Execute instrumented AX-LIB-O01 O01-E campaign"
    )
    ...
    try:
        result = run_campaign(args.plan, args.retry_policy, args.output_dir)
    except (
        KeyError,
        OSError,
        TypeError,
        ...
        failure = {
            "schema_version": "axignal.o01-history-frequency-lag-failure/v0.7",
            "status": "FAIL",
            "output": "O01_HISTORY_FREQUENCY_LAG_FAIL",
            "error_type": type(exc).__name__,
            "error": str(exc),
            "official_source_attempt_ledger": (
