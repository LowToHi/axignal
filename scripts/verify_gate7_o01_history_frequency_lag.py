...
class ContractError(RuntimeError):
    """Raised when O01-E evidence violates the frozen contract."""
...
def verify_baseline(plan: dict[str, Any]) -> None:
    ...
    require(
        baseline
        == {
            "exact_head_sha": "97a61f79f5c709ed25af2dcaf960236b711deb4f",
            "admission_output": "O01_TED_SOURCE_ADMISSION_PASS",
            "admission_artifact_id": 8838855002,
        ...
        "O01-D baseline drift",
    )
    require(closure["status"] == "PASS", "O01-D closure is not PASS")
    require(closure["output"] == baseline["admission_output"], "Admission output drift")
    ...
    require(dossier["canonical_state"] == "IN_REVIEW", "O01 state drift")
    require(dossier["claim_decision"] == "PENDING", "O01 claim drift")
...
def verify_result(result_dir: Path, plan: dict[str, Any], plan_path: Path) -> dict[str, Any]:
    names = {
    ...
    require(names <= files, "Retained O01-E evidence is incomplete")
    require(
        not any("raw" in name.casefold() or name.endswith(".zip") for name in files),
    ...
    require(history["exhaustive_notice_ingestion_performed"] is False, "Exhaustive ingest")
    ...
    require(final["status"] == "PASS", "Final O01-E result is not PASS")
    require(final["output"] == "O01_HISTORY_FREQUENCY_LAG_PASS", "Output drift")
    require(final["library_id"] == "AX-LIB-O01", "Library mismatch")
    require(final["source_id"] == "src_ted_search_api_v3", "Source mismatch")
    require(
        final["network_requests_used"] <= final["network_requests_maximum"] <= 60,
        "Network budget failed",
    )
    require(all(final["checks"].values()), "One or more O01-E checks failed")
    ...
    return {
        ...
        "plan_sha256": f"sha256:{digest(plan_path)}",
        "members": {
            name: f"sha256:{digest(result_dir / name)}" for name in sorted(names)
        },
