...
DEFAULT_PLAN = ROOT / (
    ...
    "AX-LIB-O01-history-frequency-lag-plan.v0.3.json"
)
...
def verify_plan(plan: dict, plan_path: Path) -> dict:
    require(
        plan["schema_version"] == "axignal.o01-history-frequency-lag-plan/v0.3",
    ...
    require(
        probe["artifact_id"] == 8839697337
        and probe["artifact_digest"]
        == "sha256:5b77b1a6975443ad9f9cb2af4668821aa508a2b2d72749ff4aa546aae25016f8"
        and probe["format"] == "XLS_OLE2"
        and probe["magic_hex"] == "d0cf11e0a1b11ae1"
        and probe["raw_body_retained"] is False,
    ...
    require(boundary["public_claim_contribution"] is False, "Claims enabled")
    require(boundary["global_coverage_claim"] is False, "Global claim enabled")
    require(boundary["gate7_closed"] is False, "Gate 7 closed")
    require(boundary["public_launch"] == "NO_GO", "Launch enabled")

    return {
        "status": "PASS",
        "output": "O01_HISTORY_FREQUENCY_LAG_PLAN_PASS",
        "implementation": "NUMERIC_XLS_NO_SORT_V0_4",
...
def _require_no_sort(query: str) -> None:
    require("SORT" not in query.upper(), f"Forbidden sort clause in query: {query}")
    require("publication-date >=" in query, "Missing lower publication-date bound")
    require("publication-date <=" in query, "Missing upper publication-date bound")
...
def verify_result(result_dir: Path, plan: dict, plan_path: Path) -> dict:
    ...
    require(calendars["release_calendar_bodies_persisted"] is False, "XLS retained")
    ...
    for calendar in calendars["calendars"]:
        ...
        require(
            calendar["parser_contract"]
            == "NUMERIC_ISSUE_COLUMN_0_TEXT_DATE_COLUMN_1",
            "Observed parser contract drift",
        )
        ...
        require(calendar["release_count"] == 255, "Calendar edition count drift")
        require(calendar["first_issue"] == 1, "Calendar first issue drift")
        require(calendar["last_issue"] == 255, "Calendar last issue drift")

    require(
        history["query_contract"] == "CANONICAL_CLOSED_INTERVAL_WITHOUT_SORT",
        "History query contract drift",
    )
    for observation in history["binary_search_observations"]:
        _require_no_sort(observation["query"])
    for observation in frequency["recent_release_sample"]:
        _require_no_sort(observation["query"])

    require(final["release_calendar_bodies_persisted"] is False, "Final XLS retention")
    require(final["checks"]["history_contract_diagnostic"] is True, "Diagnostic check")
    require(final["checks"]["calendar_bodies_not_persisted"] is True, "Retention check")
    return {
        **common,
        "implementation": "NUMERIC_XLS_NO_SORT_V0_4",
        "parser_lock_sha256": f"sha256:{digest(PARSER_LOCK)}",
        "calendar_probe_artifact_id": plan["calendar_format_probe"]["artifact_id"],
        "history_diagnostic_artifact_id": plan["history_contract_diagnostic"][
            "artifact_id"
        ],
        "all_queries_without_sort": True,
    }
