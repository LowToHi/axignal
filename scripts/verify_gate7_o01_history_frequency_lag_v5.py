...
DEFAULT_PLAN = ROOT / (
    ...
    "AX-LIB-O01-history-frequency-lag-plan.v0.4.json"
)
...
def verify_plan(plan: dict, plan_path: Path) -> dict:
    require(
        plan["schema_version"] == "axignal.o01-history-frequency-lag-plan/v0.4",
    ...
    require(boundary["full_internal_archive_access"] is False, "Archive enabled")
    require(boundary["exhaustive_notice_ingestion"] is False, "Exhaustive ingest enabled")
    require(boundary["exact_first_seen_timestamp_claim"] is False, "First-seen claim")
    ...
    return {
        ...
        "implementation": "NUMERIC_XLS_NO_SORT_V0_4_EXACT_AUTHORITY",
...
def verify_result(result_dir: Path, plan: dict, plan_path: Path) -> dict:
    result = verify_v4_result(result_dir, plan, plan_path)
    require(
