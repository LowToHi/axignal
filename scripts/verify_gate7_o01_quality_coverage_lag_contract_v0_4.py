...
from materialize_gate7_o01_quality_coverage_lag_plan_v0_4 import materialize_plan
...
def verify_plan(path: Path) -> dict[str, Any]:
    ...
    require(actual == expected, "Materialized v0.4 plan drift")
    ...
    require(source["scope"] == "ALL", "Historical scope is not ALL")
    ...
    return {
        "status": "PASS",
        "output": "O01_QUALITY_COVERAGE_LAG_V0_4_PLAN_FROZEN",
...
def verify_final(path: Path, result_dir: Path) -> dict[str, Any]:
    ...
    criteria = {
        ...
        "HISTORICAL_SCOPE_ALL": plan["source"]["scope"] == "ALL",
