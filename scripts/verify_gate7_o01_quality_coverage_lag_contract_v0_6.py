...
class ContractError(RuntimeError):
    """Raised when the v0.6 contract cannot be evaluated."""
...
def verify_plan(path: Path) -> dict[str, object]:
    ...
    if result.get("campaign_id") != "AX-LIB-O01-TED-QALAG-ML-CONTROLS-v0.6":
        raise ContractError("Campaign identity drift")
    if result.get("source_scope") != "ALL":
        raise ContractError("Historical scope regressed")
    if plan["sampling"]["query_contract"] != QUERY:
        raise ContractError("Canonical query drift")
    if plan["sampling"]["check_query_syntax"] is not False:
        raise ContractError("Query execution is not enabled")
    result["output"] = "O01_QUALITY_COVERAGE_LAG_V0_6_PLAN_FROZEN"
    result["query_contract"] = QUERY
    result["check_query_syntax"] = False
    return result
...
def verify_final(path: Path, result_dir: Path) -> dict[str, object]:
    ...
    plan = json.loads(path.read_text(encoding="utf-8"))
    if plan["sampling"]["check_query_syntax"] is not False:
        raise ContractError("Final evidence used syntax-check mode")
    result["query_contract"] = QUERY
    result["check_query_syntax"] = False
    result["query_execution_enabled"] = True
    return result
