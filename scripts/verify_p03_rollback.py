...
PLAN_PATH = ROOT / "data/security/p03-rollback-plan.v0.1.json"
...
assert plan["task_id"] == "AX-GE2E-P03-T01"
...
for path in plan["preserved_authority_paths"]:
    ...
    assert baseline_blob == current_blob, f"P02 authority drift detected: {path}"

with tempfile.TemporaryDirectory(prefix="axignal-p03-rollback-") as temp_dir:
    ...
    json.dumps(
        {
            ...
            "removed_p03_artifacts": len(plan["remove_paths"]),
            "restored_baseline_files": len(plan["restore_paths"]),
            "preserved_p02_authority_files": len(
                plan["preserved_authority_paths"]
            ),
            "residual_paths": 0,
