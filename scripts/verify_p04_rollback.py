...
PLAN_PATH = ROOT / "data/connectors/p04-rollback-plan.v0.1.json"
...
assert plan["task_id"] == "AX-GE2E-P04-T01"
...
for path in plan["preserved_p03_authority_paths"]:
    baseline_blob = run_git("rev-parse", f"{baseline}:{path}")
    current_blob = run_git("rev-parse", f"HEAD:{path}")
    assert baseline_blob == current_blob, f"P03 authority drift detected: {path}"

with tempfile.TemporaryDirectory(prefix="axignal-p04-rollback-") as temp_dir:
    ...
    for relative in plan["remove_paths"]:
        ...
        assert target.exists() or target.is_symlink(), (
            f"rollback target absent: {relative}"
        )
    ...
    json.dumps(
        {
            ...
            "removed_p04_artifacts": len(plan["remove_paths"]),
            "restored_baseline_files": len(plan["restore_paths"]),
            "preserved_p03_authority_files": len(
                plan["preserved_p03_authority_paths"]
            ),
            "residual_paths": 0,
