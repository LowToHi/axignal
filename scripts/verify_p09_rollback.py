...
PLAN_PATH = ROOT / "data/grants/p09-rollback-plan.v0.1.json"
...
preserved_before = {
    ...
    for path in plan["preserved_p08_authority_files"]
}

for relative in plan["p09_only_artifacts"]:
    path = ROOT / relative
    assert path.is_file(), f"missing P09 artifact before rollback: {relative}"
    path.unlink()
...
for relative in plan["p09_only_artifacts"]:
    assert not (ROOT / relative).exists(), f"residual P09 artifact: {relative}"
...
assert preserved_before == preserved_after, (
    "P08 authority drift during P09 rollback"
)
...
assert diff.returncode == 0, (
    "rolled-back tree differs from frozen P08 baseline"
)
...
result = {
    ...
    "removed_p09_artifacts": len(plan["p09_only_artifacts"]),
    "restored_baseline_files": len(plan["restored_baseline_files"]),
    "preserved_p08_authority_files": len(
        plan["preserved_p08_authority_files"]
    ),
    "residual_paths": 0,
