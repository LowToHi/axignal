...
PATHS = {
    "runtime_schema": (
        ROOT / "schemas/energy-climate-transition-workspace-runtime.schema.json"
    ),
    "fixtures_schema": (
        ROOT / "schemas/energy-climate-transition-workspace-fixtures.schema.json"
    ),
    "cases_schema": (
        ROOT / "schemas/energy-climate-transition-workspace-cases.schema.json"
    ),
    "runtime": (
        ROOT
        / "data/energy-climate/energy-climate-transition-workspace-runtime.v0.1.json"
    ),
    "fixtures": (
        ROOT / "data/energy-climate/p15-conformance-fixtures.v0.1.json"
    ),
...
task = next(
    ...
    if item["task_id"] == "AX-GE2E-P15-T01"
)
...
assert (
    runtime["dependency_status"]["p14_engineering_head"]
