...
PROFILE = (
    ROOT
    / "data/universes/eu-public-procurement"
...
def main() -> None:
    ...
    require(
        'ConfigDict(extra="forbid")' in request_contract,
        "extra fields are accepted",
    )
    require(
        "tenant_id:" not in request_contract,
        "client tenant field entered request schema",
    )
    require(
    ...
    require(
        "Depends(require_identity)" in api,
        "authenticated identity dependency is absent",
    )
    require(
        "tenant_id=identity.tenant_id" in api,
        "tenant is not resolved from identity",
    )

    require(
        MAX_ASSERTION_TTL_SECONDS <= 300,
        "identity assertion lifetime exceeds five minutes",
    )
    require(
        "hmac.compare_digest" in identity,
        "identity signature is not constant-time compared",
    )
    require(
        "ASSERTION_AUDIENCE" in identity,
        "identity audience binding is absent",
    )
    require(
        "expires_at" in identity and "issued_at" in identity,
        "identity expiry checks are absent",
    )

    require(
        "SET LOCAL ROLE" in repository,
        "database role is not transaction-scoped",
    )
    require(
        "set_config('app.tenant_id'" in repository,
        "tenant RLS context is absent",
    )
    require(
        "FORCE ROW LEVEL SECURITY" in research_sql,
        "tenant tables do not force RLS",
    )
    require(
    ...
    require(
        "sanitised_projection(page)" in ted_repository,
        "raw TED payload can enter storage",
    )
    require(
        '"api_redistribution": False' in ted_repository,
        "redistribution guard is absent",
    )
    require(
        '"model_calls": 0' in ted_repository,
        "model-free authority evidence is absent",
    )
    require(
        'source.get("kill_switch")' in worker,
        "source kill switch is not enforced",
    )

    require(
        pilot["networks"]["backend"]["internal"] is True,
        "backend is not internal",
    )
    require(
        pilot["networks"]["ted-egress"]["internal"] is False,
        "TED egress is closed",
    )
    egress_services = {
        service_id
    ...
    require(
        egress_services == {"research-worker"},
        "TED egress is not worker-exclusive",
    )
    require(
        services["api"]["environment"]["AXIGNAL_LIVE_SOURCES_ENABLED"]
        == "false",
        "global live sources were enabled in API",
    )
    require(
        services["research-worker"]["environment"]
    ...
    require(
        ...
        ["AXIGNAL_TED_PROCUREMENT_UI_ENABLED"]
        == "true",
    ...
    require(
        "api_redistribution_allowed', false" in ted_sql,
        "SQL redistribution guard is absent",
    )
    ...
    result = {
        ...
        "residual_risk": {
            "identity_assertion_replay_window_seconds": (
                MAX_ASSERTION_TTL_SECONDS
            ),
            "acceptance": (
                "PRIVATE_INTERNAL_GATEWAY_ONLY; "
                "TLS AND NETWORK ISOLATION REQUIRED"
            ),
