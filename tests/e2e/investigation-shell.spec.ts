...
test("preserves the canonical Navigator → lens → evidence workflow", async ({
  page,
}) => {
  await page.goto("/");
  ...
  const globe = page.getByLabel("Globe centrado en Moscú");
  await globe.scrollIntoViewIfNeeded();
  await expect(globe.getByText("Moscú, Rusia")).toBeVisible();
  ...
  await expect(
    page.getByRole("button", { name: /Distrito de Ramenki ALTA/ }),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(/tasas hipotecarias pueden reducir la demanda/i),
  ).toBeVisible();
});
